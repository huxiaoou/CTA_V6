...
class CCfgFactorGrpONR2(CCfgFactorGrpWin):
    def __init__(self, **kwargs):
        super().__init__(factor_class="ONR2", **kwargs)
...
class CFactorONR2(CFactorsByInstru):
    def __init__(self, factor_grp: CCfgFactorGrpONR2, **kwargs):
        if not isinstance(factor_grp, CCfgFactorGrpONR2):
            raise TypeError("factor_grp must be CCfgFactorGrpONR2")
    ...
    def cal_factor_by_instru(self, instru: str, bgn_date: str, stp_date: str, calendar: CCalendar) -> pd.DataFrame:
        ...
        major_data = self.load_preprocess(
            ...
            values=["trade_date", "ticker_major", "open_major", "pre_close_major"],
        )
        major_data["onr2"] = robust_ret_alg(
            x=major_data["open_major"],
            y=major_data["pre_close_major"],
        )
        for win, name_vanilla in zip(self.cfg.args.wins, self.cfg.names_vanilla):
            major_data[name_vanilla] = major_data["onr2"].rolling(window=win).sum()
        w0, w1 = 240, 5
